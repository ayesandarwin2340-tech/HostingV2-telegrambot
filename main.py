        ...
        if os.path.exists(package_json_path):
            ...
            try:
                ...
                if not os.path.exists(node_modules_path):
                    bot.send_message(script_owner_id, f"📦 Installing dependencies from package.json...", parse_mode='Markdown')
                    ...
                    if install_process.returncode == 0:
                        ...
                        bot.send_message(script_owner_id, f"✅ Dependencies installed successfully", parse_mode='Markdown')
                    else:
                        logger.error(f"❌ npm install failed: {install_process.stderr}")
                        bot.send_message(script_owner_id, f"❌ Failed to install dependencies:\n```\n{install_process.stderr[:200]}\n```", parse_mode='Markdown')
            except subprocess.TimeoutExpired:
                logger.error("❌ npm install timed out")
                bot.send_message(script_owner_id, f"❌ npm install timed out", parse_mode='Markdown')
            except Exception as e:
                logger.error(f"❌ Error during npm install: {e}")

        # Check for missing modules by analyzing the file
        if attempt == 1:
            try:
                ...
                for module in imports:
                    # Skip relative paths and core modules
                    if module.startswith('.') or module.startswith('/') or module in core_modules:
                        continue
                ...
                if missing_modules:
                    ...
                    for module in missing_modules[:5]:  # Install max 5 at a time
                        try:
                            bot.send_message(script_owner_id, f"📦 Installing `{module}`...", parse_mode='Markdown')
                            ...
                            if install_result.returncode == 0:
                                ...
                                bot.send_message(script_owner_id, f"✅ Installed `{module}`", parse_mode='Markdown')
                            else:
                                logger.error(f"❌ Failed to install {module}: {install_result.stderr}")
                                bot.send_message(script_owner_id, f"❌ Failed to install `{module}`", parse_mode='Markdown')
                                
                        except subprocess.TimeoutExpired:
                            logger.error(f"❌ Timeout installing {module}")
                            bot.send_message(script_owner_id, f"❌ Timeout installing `{module}`", parse_mode='Markdown')
                        except Exception as e:
