...
def get_node_version():
    """Get Node.js version with multiple detection methods"""
    try:
        # Method 1: Try 'node' command
    ...
    try:
        # Method 2: Try 'which node' to find path
        which_result = subprocess.run(['which', 'node'], capture_output=True, text=True, timeout=5)
        if which_result.returncode == 0:
            node_path = which_result.stdout.strip()
            version_result = subprocess.run([node_path, '--version'], capture_output=True, text=True, timeout=5)
            if version_result.returncode == 0:
                return version_result.stdout.strip()
    except:
        pass
    
    # Method 3: Check common Railway paths
    common_paths = [
        '/usr/bin/node',
        '/usr/local/bin/node',
        '/opt/railway/node/bin/node',
        '/app/.railway/node/bin/node',
        '/opt/buildhome/.nvm/versions/node/v18/bin/node',
        '/opt/buildhome/.nvm/versions/node/v20/bin/node'
    ]
...
class ScriptRunner:
    ...
    def _detect_node(self):
        """Detect Node.js installation at startup"""
        try:
            # Method 1: which node
            which_result = subprocess.run(['which', 'node'], capture_output=True, text=True, timeout=5)
            if which_result.returncode == 0:
                self.node_path = which_result.stdout.strip()
                version = subprocess.run([self.node_path, '--version'], capture_output=True, text=True, timeout=5)
                if version.returncode == 0:
                    logger.info(f"✅ Node.js found at {self.node_path}, version: {version.stdout.strip()}")
                    return True
            ...
            for node_cmd in common_paths:
                try:
                    result = subprocess.run([node_cmd, '--version'], capture_output=True, text=True, timeout=5)
                    if result.returncode == 0:
                        self.node_path = node_cmd
                        logger.info(f"✅ Node.js found at {node_cmd}, version: {result.stdout.strip()}")
                        return True
                except:
                    continue

            logger.warning("⚠️ Node.js not found. JavaScript files will not run.")
            return False
        except Exception as e:
            logger.error(f"Error detecting Node.js: {e}")
            return False
    ...
    def _execute_script(self, script_info):
            ...
            if not self.node_path:
                if chat_id:
                    bot.send_message(chat_id, 
                        ...
                        "JavaScript files cannot be executed.\n"
                        "Please upload Python (.py) files only.",
                        parse_mode='Markdown'
                    )
                return
        ...
        try:
            ...
            if file_type == 'js' and os.path.exists(os.path.join(user_folder, 'node_modules')):
        ...
        if os.path.exists(package_json):
            def install():
                try:
                    ...
                    npm_path = 'npm'
                    try:
                        which_npm = subprocess.run(['which', 'npm'], capture_output=True, text=True, timeout=5)
                        if which_npm.returncode == 0:
                            npm_path = which_npm.stdout.strip()
                    except:
                        pass
                    
                    result = subprocess.run(
                        [npm_path, 'install', '--cache', NPM_CACHE_DIR],
                        cwd=user_folder,
                        capture_output=True,
                        text=True,
                        timeout=300
        ...
        with self.install_lock:
            try:
                if pkg_type == 'python':
                    ...
                    result = subprocess.run(
                        [npm_path, 'install', '--cache', NPM_CACHE_DIR, package],
                        cwd=user_folder,
                        capture_output=True,
                        text=True,
                        timeout=300
...
def handle_stats(message):
    ...
    with db_manager.get_connection() as conn:
        ...
        running_scripts_db = running_result[0] if running_result else 0
