...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    if attempt > max_attempts:
        try:
            bot.send_message(script_owner_id, f"❌ Failed to start `{file_name}` after {max_attempts} attempts", parse_mode='Markdown')
        except Exception as e:
            logger.error(f"❌ Failed to send error message: {e}")
        return
    ...
    try:
        ...
        try:
            ...
            bot.send_message(script_owner_id, f"❌ Node.js is not installed on the server", parse_mode='Markdown')
            return
        except subprocess.TimeoutExpired:
            bot.send_message(script_owner_id, f"❌ Node.js check timed out", parse_mode='Markdown')
            return
        ...
        if os.path.exists(package_json_path) and attempt == 1:
            ...
            try:
                ...
                if not os.path.exists(node_modules_path):
                    status_msg = bot.send_message(script_owner_id, f"📦 Installing dependencies from package.json...\nThis may take a few minutes...", parse_mode='Markdown')
                    
                    # Run npm install with longer timeout
                    install_process = subprocess.run(
                        ['npm', 'install', '--no-audit', '--no-fund'],  # Skip audit to speed up
                        cwd=user_folder,
                        capture_output=True,
                        text=True,
                        env=current_env,
                ...
                bot.send_message(script_owner_id, f"❌ npm install timed out after 5 minutes.\nTry installing manually or check your package.json", parse_mode='Markdown')
                return
            except Exception as e:
        ...
        # For subsequent attempts or if no package.json, check individual modules
        if attempt == 1:
            try:
                ...
                # Filter out relative imports and core modules
                core_modules = ['fs', 'path', 'http', 'https', 'url', 'util', 'events', 
                               'stream', 'crypto', 'zlib', 'os', 'child_process', 'buffer',
                               'timers/promises']  # Added timers/promises
                
                # Special handling for built-in modules with slashes
                builtin_patterns = ['timers/', 'stream/', 'fs/', 'path/']
                ...
                for module in imports:
                    ...
                    # Check for built-in modules with slashes
                ...
                if missing_modules:
                    ...
                    for module in missing_modules[:3]:  # Install 3 at a time max
                        try:
                            status_msg = bot.send_message(script_owner_id, f"📦 Installing `{module}`...\nThis may take a moment...", parse_mode='Markdown')
                            
                            install_result = subprocess.run(
                                ['npm', 'install', module, '--no-audit', '--no-fund'],
                                cwd=user_folder,
                                capture_output=True,
                                text=True,
                                env=current_env,
                                timeout=120  # 2 minutes per module
                            )
                    ...
                    # If we installed any modules, restart with attempt+1
                    if missing_modules:
                        bot.send_message(script_owner_id, f"🔄 Restarting `{file_name}` after module installation...", parse_mode='Markdown')
                        time.sleep(3)  # Give more time for modules to be ready
                        threading.Thread(target=run_js_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1)).start()
                        return
